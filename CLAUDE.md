...
- v2.30: **Compare tab quick-fill ergonomics — director feedback.** Two changes to reduce manual date entry:
  ...
  3. **`cmpShiftB(kind)` helper** in App component scope — single source of truth used by all 6 buttons (3 YYB + 3 TL). Splits the date string, shifts via `Date.setFullYear`/`setMonth`/`setDate`, then handles month-end overflow by clamping (e.g., Mar 31 → previous month → Feb 28, not the JS-default Mar 3). Week shift is a simple `-7` days with no edge-case handling needed.
  4. **i18n**: new keys `cmpPrevYear` / `cmpPrevMonth` / `cmpPrevWeek` ("Previous Year (B from A)" etc. / "前年 (BをAから)" etc.).
- v2.29: **Replaced emoji sidebar icons with custom SVG line icons.** New `src/icons.jsx` file exports 18 icon components (14×14, stroke-based, `currentColor` inheritance). All 31 tab emoji icons replaced with clean SVG equivalents that automatically match active/inactive state and dark/light theme colors. Fixed `tl-cancellations` which was previously missing an icon. Icon rendering span updated from `fontSize:12` text centering to flex centering for proper SVG alignment.
...
- v2.22: **New "Stay Night" date type option.** Added as a 4th choice in the global Date Type dropdown (YYB and TL both expose it). Unlike the existing point-in-time options (check-in / check-out / booking date), Stay Night uses an **overlap filter**: a reservation is included if ANY of its stay nights falls within the date range — i.e. `checkin ≤ to AND checkout > from` (checkout night isn't counted as occupied). Applied to the main `filtered` memo, `compareRpt`/`cancelRpt`/`paceRpt` `getDateStr`/`inRange` helpers, the `losRpt` inline date filter, and the TL-side filter (`tlFiltered` / `tlAllStatusFiltered`) and `tlCancelRpt.apply`. For aggregation (daily/DOW charts), `getDateField()` uses `r.checkin` as the anchor date in stay mode — night-level distribution would require exploding each reservation into N rows, out of scope for this change. New i18n key `t.stayNight` ("Stay Night" / "宿泊日").
  ...
  **Discussion deferred: true per-night attribution.** In v2.22 a reservation spanning Feb 28 → Mar 2 is shown *whole* when filtering on "March" — all revenue, 2 nights, DOW-at-checkin etc. attributed to March totals. The user pointed out this is wrong: only the Mar 1 night (and its proportional revenue) should show up. Implementing true per-night attribution means exploding each reservation into one row per stay night. This collides with several per-booking metrics (Reservations count, LOS/Lead averages, Cancellations, DOW of check-in, Repeater analysis) which would double-count if naively computed on exploded rows. Tabled for now. Three approaches under consideration when we come back to it:
  1. **Partial fix** (~half day): Explode only for daily/monthly/DOW/revenue time-series charts; per-reservation metrics continue using the current whole-reservation overlap semantics. Each chart labels its unit (room-night vs reservation) so the distinction is explicit.
  2. **Full fix** (~1–2 days): Maintain two data views — `filteredNights` (exploded) and `filteredBookings` (per-reservation) — wire every memo to the correct one, audit all tabs. Most correct but largest blast radius.
  3. **Minimal fix** (~1 hour): Keep the current overlap filter but add a `stayNightsInRange` field to each filtered row (`max(checkin, from) → min(checkout, to+1)`). Revenue/room-night displays use a proportional value (`totalRev × nightsInRange / totalNights`). Daily charts still plot at check-in date so Mar 1's bar doesn't light up for a Feb 28 check-in, but totals reconcile correctly.
...
- v2.13: **Added Premium MONday 浅草 Ⅰ.** New facility appeared in YYB data (2 advance bookings). Rooms: 26. Opening: 2026-05-01 (later revised to 2026-06-01 in v2.27). Source: Excel 250430JHAT Property List, row 50 (codename 西浅草ノース). Added alias `"Premium MONday 浅草 ONE" → "Premium MONday 浅草 Ⅰ"` since brand-strategy deck previously used "ONE" while data uses the Roman numeral Ⅰ. Later added `"MONday Premium 浅草 ONE"` and `"MONday Premium 浅草 Ⅰ"` aliases for the reversed word-order variant that appeared in data.
