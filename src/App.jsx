...
export default function App(){
  ...
  const DG=({tabId,children})=>{const saved=loadLayouts(tabId);const layouts=saved||DL[tabId];const[cRef,width]=useContainerWidth();return(<div ref={cRef}><div style={{display:"flex",justifyContent:"flex-end",marginBottom:6}}>{saved&&<button style={{...S.btn,fontSize:10}} onClick={()=>{clearLayout(tabId);setLayoutVer(v=>v+1)}}>{t.resetLayout}</button>}</div>{width>0&&<Responsive key={tabId+layoutVer} width={width} {...RGL_PROPS} layouts={layouts} onLayoutChange={(_,all)=>saveLayouts(tabId,all)}>{children}</Responsive>}</div>)};
  ...
  return(
    ...
    <div style={S.inner}>
      ...
      {!agg?<div style={{textAlign:"center",color:"#a0977f",padding:40}}>{t.noData}</div>:<>
        ...
        {/* OVERVIEW */}
        {tab==="overview"&&<>{insights.overview&&<div style={{...S.insight,whiteSpace:"pre-line"}}>{insights.overview}</div>}<DG tabId="overview">
        ...
        {/* MARKETS */}
        {tab==="markets"&&<div>{insights.markets&&<div style={{...S.insight,whiteSpace:"pre-line"}}>{insights.markets}</div>}<DG tabId="markets">
        ...
        {/* BOOKING */}
        {tab==="booking"&&<>{insights.booking&&<div style={{...S.insight,whiteSpace:"pre-line"}}>{insights.booking}</div>}<DG tabId="booking">
        ...
        {/* REVENUE */}
        {tab==="revenue"&&<>{insights.revenue&&<div style={{...S.insight,whiteSpace:"pre-line"}}>{insights.revenue}</div>}<DG tabId="revenue">
        ...
        {/* ROOMS */}
        {tab==="rooms"&&<>{insights.rooms&&<div style={{...S.insight,whiteSpace:"pre-line"}}>{insights.rooms}</div>}<DG tabId="rooms">
