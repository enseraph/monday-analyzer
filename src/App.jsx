...
function dlChart(id,fn){const el=document.getElementById(id);if(!el)return;const svg=el.querySelector("svg");if(!svg)return;const c=svg.cloneNode(true);c.setAttribute("xmlns","http://www.w3.org/2000/svg");const d=new XMLSerializer().serializeToString(c);const cv=document.createElement("canvas");const ctx=cv.getContext("2d");const img=new Image();const u=URL.createObjectURL(new Blob([d],{type:"image/svg+xml;charset=utf-8"}));img.onload=()=>{cv.width=img.width*2;cv.height=img.height*2;ctx.scale(2,2);ctx.fillStyle="#ffffff";ctx.fillRect(0,0,img.width,img.height);ctx.drawImage(img,0,0);const a=document.createElement("a");a.download=fn+".png";a.href=cv.toDataURL("image/png");a.click();URL.revokeObjectURL(u)};img.src=u}

function dlTable(data,title,fn){if(!data||!data.length)return;const keys=Object.keys(data[0]);const pad=12,rowH=28,headH=36,titleH=40,font="12px 'DM Sans',sans-serif",headFont="bold 11px 'JetBrains Mono',monospace",titleFont="bold 14px 'DM Sans',sans-serif";const cv=document.createElement("canvas");const ctx=cv.getContext("2d");ctx.font=font;const colW=keys.map(k=>{let mx=ctx.measureText(k.toUpperCase()).width;data.forEach(r=>{const w=ctx.measureText(String(r[k]??"")).width;if(w>mx)mx=w});return mx+pad*2});const totalW=colW.reduce((a,b)=>a+b,0)+2;const totalH=titleH+headH+data.length*rowH+2;cv.width=totalW*2;cv.height=totalH*2;ctx.scale(2,2);ctx.fillStyle="#ffffff";ctx.fillRect(0,0,totalW,totalH);ctx.fillStyle="#1a1a2e";ctx.font=titleFont;ctx.fillText(title,pad,titleH-12);ctx.fillStyle="#f0f0f4";ctx.fillRect(0,titleH,totalW,headH);ctx.fillStyle="#4a4a6a";ctx.font=headFont;let x=1;keys.forEach((k,i)=>{ctx.fillText(k.toUpperCase(),x+pad,titleH+headH-10);x+=colW[i]});data.forEach((row,ri)=>{const y=titleH+headH+ri*rowH;if(ri%2===0){ctx.fillStyle="#fafaff";ctx.fillRect(0,y,totalW,rowH)}ctx.fillStyle="#333";ctx.font=font;let x2=1;keys.forEach((k,i)=>{const v=row[k];const s=v==null?"":typeof v==="number"?v.toLocaleString():String(v);ctx.fillText(s,x2+pad,y+rowH-8);x2+=colW[i]})});ctx.strokeStyle="#e0e0e8";ctx.lineWidth=0.5;let lx=1;keys.forEach((_,i)=>{lx+=colW[i];ctx.beginPath();ctx.moveTo(lx,titleH);ctx.lineTo(lx,totalH);ctx.stroke()});const a=document.createElement("a");a.download=fn+"_table.png";a.href=cv.toDataURL("image/png");a.click()}
...
export default function App(){
  ...
  const EB=({id,nm,data,title})=><div style={{display:"flex",gap:6,marginTop:4}}><button style={{...S.btn,fontSize:10}} onClick={()=>dlChart(id,nm)}>{t.exportImg}</button>{data&&data.length>0&&<button style={{...S.btn,fontSize:10}} onClick={()=>dlTable(data,title||nm,nm)}>📋 Table</button>}</div>;
  const CC=({title,id,nm,children,h,data})=><div style={S.card}><div style={S.ct}>{title}</div><div id={id}><ResponsiveContainer width="100%" height={h||280}>{children}</ResponsiveContainer></div><EB id={id} nm={nm} data={data} title={title}/></div>;
      ...
      {!agg?<div style={{textAlign:"center",color:"#a0977f",padding:40}}>{t.noData}</div>:<>
        ...
        {tab==="kvk"&&kvk&&<div>
          ...
          <div style={G}>
            ...
            <CC title={t.kvkSegByMonth} id="kk-sg-mo" nm="seg_month" data={kvk.segMo}><BarChart data={kvk.segMo}><CartesianGrid {...gl}/><XAxis dataKey="month" tick={tk}/><YAxis tick={tk}/><Tooltip content={<CT/>}/><Legend/>{SEG_ORDER.map((s,i)=><Bar key={s} dataKey={s} stackId="a" fill={SEG_COLORS[s]} name={tl(s)}/>)}</BarChart></CC>
          </div>
          <CC title={t.kvkSegByCountry} id="kk-sg-co" nm="seg_country" h={Math.max(300,kvk.segCountry.length*26)} data={kvk.segCountry}><BarChart data={kvk.segCountry} layout="vertical"><CartesianGrid {...gl}/><XAxis type="number" domain={[0,100]} tick={tks} tickFormatter={v=>v+"%"}/><YAxis dataKey="country" type="category" width={120} tick={<TlTickV/>} interval={0}/><Tooltip content={<CT formatter={v=>v+"%"}/>}/><Legend/>{SEG_ORDER.map(s=><Bar key={s} dataKey={s} stackId="a" fill={SEG_COLORS[s]} name={tl(s)}/>)}</BarChart></CC>
          ...
          {/* 5. REVENUE */}
          <div style={G}>
            <CC title={t.kvkADRBySeg} id="kk-adr" nm="adr_seg" data={kvk.adrSeg}><BarChart data={kvk.adrSeg}><CartesianGrid {...gl}/><XAxis dataKey="segment" tick={<TlTick/>}/><YAxis tick={tk} tickFormatter={fmtY}/><Tooltip content={<CT formatter={v=>"¥"+v.toLocaleString()}/>}/><Bar dataKey="adr" radius={[4,4,0,0]} name="ADR">{kvk.adrSeg.map((e,i)=><Cell key={i} fill={SEG_COLORS[e.segment]||PALETTE[i]}/>)}</Bar></BarChart></CC>
