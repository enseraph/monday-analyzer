...
// Internal sentinel to avoid collision with a real country literally named "Other" (COUNTRY_MAP used to remap "その他"/"Other" to "Other").
// Charts display this via tl() → t.otherLabel ("Other" / "その他").
const OTHER_KEY_NAME="__MA_OTHER__"; // country/facility/channel label for the aggregate bucket
...
const T = {
  en: {
    ...
    cumRevByNationality:"Cumulative Revenue by Nationality",cmpCumRevByNationality:"Cumulative Revenue by Nationality (A vs B)",
...
cmpMonthVsMonth:"Month vs Month",cmpWeekVsWeek:"Week vs Week",cmpYearVsYear:"Year vs Year",cmpPrevYear:"Prev Year →B",cmpPrevMonth:"Prev Month →B",cmpPrevWeek:"Prev Week →B",cmpMonthVsMonthTip:"This month-to-date vs last month same length",cmpWeekVsWeekTip:"Last 7 days vs prior 7 days",cmpYearVsYearTip:"This year-to-date vs last year same length",cmpPrevYearTip:"Fill Period B with Period A shifted back 1 year",cmpPrevMonthTip:"Fill Period B with Period A shifted back 1 month",cmpPrevWeekTip:"Fill Period B with Period A shifted back 7 days",
...
otherLabel:"Other",
facAge:"Facility age",facAgeAll:"All",facAgeNew:"New only",facAgeOld:"Old only",facAgeView:"Age view",facAgeViewAgg:"Aggregate",facAgeViewSplit:"New vs Old",facAgeHint:"Applied to all facilities; selecting specific facilities overrides.",facAgeDisabled:"Specific facilities selected — age toggles disabled",facTypeView:"Type view",facTypeViewAgg:"Aggregate",facTypeViewSplit:"Hotel vs Apart",facTypeHint:"Splits compatible charts by Hotel vs Apart; the Hotel/Apart filter overrides.",facTypeDisabled:"Hotel/Apart filter is set — view toggle disabled",facHotelLabel:"Hotel",facApartLabel:"Apart",
...
segBreakdownMode:"Breakdown",segSimple:"Simple",segDetailedLabel:"Detailed",
  ...
  ja: {
    ...
    cumRevByNationality:"国別累積売上",cmpCumRevByNationality:"国別累積売上（A vs B）",
...
cmpMonthVsMonth:"月 vs 月",cmpWeekVsWeek:"週 vs 週",cmpYearVsYear:"年 vs 年",cmpPrevYear:"前年 →B",cmpPrevMonth:"前月 →B",cmpPrevWeek:"前週 →B",cmpMonthVsMonthTip:"今月（月初〜本日）vs 先月の同期間",cmpWeekVsWeekTip:"直近7日 vs その前の7日",cmpYearVsYearTip:"今年（年初〜本日）vs 昨年の同期間",cmpPrevYearTip:"期間Aを1年前にずらして期間Bに入力",cmpPrevMonthTip:"期間Aを1ヶ月前にずらして期間Bに入力",cmpPrevWeekTip:"期間Aを7日前にずらして期間Bに入力",
...
otherLabel:"その他",
facAge:"施設年代",facAgeAll:"全て",facAgeNew:"新規のみ",facAgeOld:"既存のみ",facAgeView:"年代表示",facAgeViewAgg:"合計",facAgeViewSplit:"新規vs既存",facAgeHint:"全施設対象。施設を個別選択した場合はこちらが優先。",facAgeDisabled:"施設を個別選択中 — 年代フィルターは無効",facTypeView:"タイプ表示",facTypeViewAgg:"合計",facTypeViewSplit:"ホテルvsアパート",facTypeHint:"ホテル/アパートで分割表示。ホテル/アパートフィルターが優先。",facTypeDisabled:"ホテル/アパートフィルター適用中 — 表示切替は無効",facHotelLabel:"ホテル",facApartLabel:"アパート",
...
export default function App(){
  ...
  const tlBase=useMemo(()=>({"Kanto":t.kanto,"Kansai":t.kansai,"Solo":t._Solo,"Couple":t._Couple,"Family":t._Family,"Group":t._Group,"Hotel":t._Hotel,"Apart":t._Apart,"No Rank":t._NoRank,"Regular":t._Regular,"Gold":t._Gold,"Platinum":t._Platinum,"返金不可":t._PlanNR,"学生":t._PlanStudent,"その他":t._PlanOther,"__MA_OTHER__":t.otherLabel,"__other__":t.otherLabel,"Other":t.otherLabel}),[t]);
...
// v2.32 Hotel vs Apart split colors — deliberately distinct from Compare (blue/amber), Age (green/slate), Country palette
const HOTEL_COLOR="#7c3aed"; // vivid purple — "tower / building"
const APART_COLOR="#f97316"; // warm orange — "residential / homely"
...
const uDOW=useMemo(()=>DOW_FULL,[]);
  ...
  // ─── v2.32: YYB Hotel vs Apart split — active only when facTypeView==="hotelVsApart" AND fHType==="All" ───
  const facTypeSeries=useMemo(()=>{
    if(tab!=="overview"&&tab!=="revenue"&&tab!=="booking")return null;
    if(facTypeView!=="hotelVsApart"||fHType!=="All"||!filtered.length)return null;
    const byMonth={},byDate={},byDow={};
    filtered.forEach(r=>{
      const bucket=r.hotelType==="Hotel"?"Hotel":"Apart";
      const m=getM(r);
      const d=tzFmt(getDateField(r));
      const dow=getDowField(r);
      const rev=r.totalRev||0;
      if(m){if(!byMonth[m])byMonth[m]={Hotel:{count:0,rev:0},Apart:{count:0,rev:0}};byMonth[m][bucket].count++;byMonth[m][bucket].rev+=rev}
      if(d){if(!byDate[d])byDate[d]={Hotel:{count:0,rev:0},Apart:{count:0,rev:0}};byDate[d][bucket].count++;byDate[d][bucket].rev+=rev}
      if(dow){if(!byDow[dow])byDow[dow]={Hotel:{count:0,rev:0},Apart:{count:0,rev:0}};byDow[dow][bucket].count++;byDow[dow][bucket].rev+=rev}
    });
    const row=(map,key,xField,xVal,metric)=>({[xField]:xVal,Hotel:map[key]?.Hotel?.[metric]||0,Apart:map[key]?.Apart?.[metric]||0});
    const mKeys=Object.keys(byMonth).sort(),dKeys=Object.keys(byDate).sort();
    return{
      monthlyCount:mKeys.map(m=>row(byMonth,m,"month",m,"count")),
      monthlyRev:mKeys.map(m=>row(byMonth,m,"month",m,"rev")),
  ...
  },[tab,facTypeView,fHType,filtered,monthMode,tz,fDT,tzFmt,dL]);
  ...
  // ─── v2.32: TL Hotel vs Apart split — mirrors facTypeSeries for TL charts ───
  const tlTypeSeries=useMemo(()=>{
    if(facTypeView!=="hotelVsApart"||fTlHotelType!=="All"||!tlFiltered.length)return null;
    const byMonth={},byDate={},byDow={};
    tlFiltered.forEach(r=>{
      const bucket=r.hotelType==="Hotel"?"Hotel":"Apart";
      const m=tlGetM(r);
  ...
  },[facTypeView,fTlHotelType,tlFiltered,monthMode,fDT]);
  ...
  const SortTbl=({data,columns,renderRow,grandTotalRow,title,exportFn})=>{
    ...
    return<div style={S.card}>
      ...
      <div style={{overflowX:"auto"}}><table style={S.tbl}><thead><tr>
        {columns.map(c=><th key={c.key} style={{...S.th,cursor:"pointer",...(c.headerColor?{color:c.headerColor}:{})}} onClick={()=>toggleSort(c.key)}>{c.label}{arrow(c.key)}</th>)}
      </tr></thead><tbody>
  ...
  return(
    ...
    <div style={S.inner}>
      ...
      {filtersOpen?<div style={{...S.card,overflow:"visible",marginBottom:16,display:"flex",gap:8,alignItems:"flex-end",flexWrap:"wrap",position:"sticky",top:0,zIndex:50,background:TH.filterBg,backdropFilter:"blur(8px)",boxShadow:"0 4px 16px rgba(0,0,0,0.4)",padding:12}}>
        ...
        <div><div style={Sc.fl} title={fHType!=="All"?t.facTypeDisabled:t.facTypeHint}>{t.facTypeView}</div><div style={{display:"flex",gap:2}}>{[["aggregate",t.facTypeViewAgg],["hotelVsApart",t.facTypeViewSplit]].map(([v,l])=><button key={v} style={{...Sc.btn,...(facTypeView===v?S.ba:{}),...(fHType!=="All"?{opacity:0.5,cursor:"not-allowed"}:{})}} onClick={()=>fHType==="All"&&setFacTypeView(v)} disabled={fHType!=="All"} title={fHType!=="All"?t.facTypeDisabled:""}>{l}</button>)}</div></div>
...
<div><div style={Sc.fl}>{t.dowFilter}</div><MS options={uDOW} selected={fDOW} onChange={setFDOW} placeholder={t.allDOW} S={Sc} cl={t.clear} theme={TH} displayFn={tl}/></div>
      ...
      {tab==="daily"&&<div>
        ...
        {tab==="compare"&&<div>
            ...
            <div style={{display:"flex",gap:3,flexWrap:"wrap"}}>
              <button style={{...S.btn,fontSize:9,padding:"3px 7px"}} title={t.cmpMonthVsMonthTip} onClick={()=>{const end=new Date();end.setDate(end.getDate()-DATA_LAG_DAYS);const y=end.getFullYear(),m=end.getMonth(),d=end.getDate();const a1=`${y}-${String(m+1).padStart(2,"0")}-01`;const a2=tzFmt(end);const days=d-1;const prevY=m===0?y-1:y;const prevM=m===0?12:m;const b1=`${prevY}-${String(prevM).padStart(2,"0")}-01`;const bEnd=new Date(prevY,prevM-1,1);bEnd.setDate(bEnd.getDate()+days);const b2=tzFmt(bEnd);setCmpA({from:a1,to:a2});setCmpB({from:b1,to:b2})}}>{t.cmpMonthVsMonth}</button>
              <button style={{...S.btn,fontSize:9,padding:"3px 7px"}} title={t.cmpWeekVsWeekTip} onClick={()=>{const end=new Date();end.setDate(end.getDate()-DATA_LAG_DAYS);const start=new Date(end);start.setDate(end.getDate()-6);const prevEnd=new Date(start);prevEnd.setDate(start.getDate()-1);const prevStart=new Date(prevEnd);prevStart.setDate(prevEnd.getDate()-6);setCmpA({from:tzFmt(start),to:tzFmt(end)});setCmpB({from:tzFmt(prevStart),to:tzFmt(prevEnd)})}}>{t.cmpWeekVsWeek}</button>
              <button style={{...S.btn,fontSize:9,padding:"3px 7px"}} title={t.cmpYearVsYearTip} onClick={()=>{const end=new Date();end.setDate(end.getDate()-DATA_LAG_DAYS);const y=end.getFullYear();const a1=`${y}-01-01`;const a2=tzFmt(end);const b1=`${y-1}-01-01`;const prevEnd=new Date(y-1,end.getMonth(),end.getDate());const b2=tzFmt(prevEnd);setCmpA({from:a1,to:a2});setCmpB({from:b1,to:b2})}}>{t.cmpYearVsYear}</button>
              <button style={{...S.btn,fontSize:9,padding:"3px 7px",borderColor:TH.gold,color:TH.gold}} title={t.cmpPrevYearTip} disabled={!cmpA.from} onClick={()=>cmpShiftB("year")}>{t.cmpPrevYear}</button>
              <button style={{...S.btn,fontSize:9,padding:"3px 7px",borderColor:TH.gold,color:TH.gold}} title={t.cmpPrevMonthTip} disabled={!cmpA.from} onClick={()=>cmpShiftB("month")}>{t.cmpPrevMonth}</button>
              <button style={{...S.btn,fontSize:9,padding:"3px 7px",borderColor:TH.gold,color:TH.gold}} title={t.cmpPrevWeekTip} disabled={!cmpA.from} onClick={()=>cmpShiftB("week")}>{t.cmpPrevWeek}</button>
            </div>
          </div>
          {compareRpt&&!compareRpt.empty?<>
            ...
            <DraggableGrid {...dgProps("compare")}>
              <div key="cmp-country"><SortTbl
                ...
                columns={[{key:"country",label:t.drCountry},{key:"countA",label:"A "+t.drCount,headerColor:CMP_A_COLOR},{key:"revA",label:"A "+t.drRevenue,headerColor:CMP_A_COLOR},{key:"nightsA",label:"A "+t.roomNights,headerColor:CMP_A_COLOR},{key:"countB",label:"B "+t.drCount,headerColor:CMP_B_COLOR},{key:"revB",label:"B "+t.drRevenue,headerColor:CMP_B_COLOR},{key:"nightsB",label:"B "+t.roomNights,headerColor:CMP_B_COLOR},{key:"revDelta",label:"Δ "+t.drRevenue},{key:"nightsDelta",label:"Δ "+t.roomNights}]}
              ...
              <div key="cmp-segment"><SortTbl
                ...
                columns={[{key:"segment",label:t.thSegment},{key:"countA",label:"A "+t.drCount,headerColor:CMP_A_COLOR},{key:"revA",label:"A "+t.drRevenue,headerColor:CMP_A_COLOR},{key:"nightsA",label:"A "+t.roomNights,headerColor:CMP_A_COLOR},{key:"countB",label:"B "+t.drCount,headerColor:CMP_B_COLOR},{key:"revB",label:"B "+t.drRevenue,headerColor:CMP_B_COLOR},{key:"nightsB",label:"B "+t.roomNights,headerColor:CMP_B_COLOR},{key:"revDelta",label:"Δ "+t.drRevenue},{key:"nightsDelta",label:"Δ "+t.roomNights}]}
              ...
              <div key="cmp-facility"><SortTbl
                ...
                columns={[{key:"name",label:t.thFacility},{key:"countA",label:"A "+t.drCount,headerColor:CMP_A_COLOR},{key:"revA",label:"A "+t.drRevenue,headerColor:CMP_A_COLOR},{key:"nightsA",label:"A "+t.roomNights,headerColor:CMP_A_COLOR},{key:"countB",label:"B "+t.drCount,headerColor:CMP_B_COLOR},{key:"revB",label:"B "+t.drRevenue,headerColor:CMP_B_COLOR},{key:"nightsB",label:"B "+t.roomNights,headerColor:CMP_B_COLOR},{key:"revDelta",label:"Δ "+t.drRevenue},{key:"nightsDelta",label:"Δ "+t.roomNights}]}
      ...
      {!agg?<div style={{textAlign:"center",color:TH.textMuted,padding:40}}>{t.noData}</div>:<>
        ...
        {tab==="overview"&&<><DraggableGrid {...dgProps("overview")}>
          <div key="ch-mo"><CC grid title={t.resByMonth} id="ch-mo" nm="monthly" data={perPropertySeries?perPropertySeries.monthlyCount:facAgeSeries?facAgeSeries.monthlyCount:facTypeSeries?facTypeSeries.monthlyCount:(perCountrySeries?perCountrySeries.monthlyCount:moD)}><BarChart data={perPropertySeries?perPropertySeries.monthlyCount:facAgeSeries?facAgeSeries.monthlyCount:facTypeSeries?facTypeSeries.monthlyCount:(perCountrySeries?perCountrySeries.monthlyCount:moD)}><CartesianGrid {...gl}/><XAxis dataKey="month" tick={tk}/><YAxis tick={tk}/><Tooltip content={<CT/>}/>{(perPropertySeries||facAgeSeries||facTypeSeries||perCountrySeries)?<Legend wrapperStyle={{fontSize:9}}/>:null}{perPropertySeries?perPropertySeries.properties.map((f,i)=><Bar key={f} dataKey={f} stackId="a" fill={seriesColor(f,i)} name={seriesFacLabel(f)}/>):facAgeSeries?[<Bar key="Old" dataKey="Old" stackId="a" fill={OLD_COHORT_COLOR} name={t.facOldLabel}/>,<Bar key="New" dataKey="New" stackId="a" fill={NEW_COHORT_COLOR} name={t.facNewLabel}/>]:facTypeSeries?[<Bar key="Apart" dataKey="Apart" stackId="a" fill={APART_COLOR} name={t.facApartLabel}/>,<Bar key="Hotel" dataKey="Hotel" stackId="a" fill={HOTEL_COLOR} name={t.facHotelLabel}/>]:perCountrySeries?perCountrySeries.countries.map((c,i)=><Bar key={c} dataKey={c} stackId="a" fill={seriesColor(c,i)} name={seriesLabel(c)}/>):<Bar dataKey="count" fill="#4ea8de" radius={[4,4,0,0]} name={t.reservations}/>}</BarChart></CC></div>
          ...
          <div key="ch-mo-rev"><CC grid title={t.monthlyRev} id="ch-mo-rev" nm="monthly_rev_ov" data={perPropertySeries?perPropertySeries.monthlyRev:facAgeSeries?facAgeSeries.monthlyRev:facTypeSeries?facTypeSeries.monthlyRev:(perCountrySeries?perCountrySeries.monthlyRev:moD)}><BarChart data={perPropertySeries?perPropertySeries.monthlyRev:facAgeSeries?facAgeSeries.monthlyRev:facTypeSeries?facTypeSeries.monthlyRev:(perCountrySeries?perCountrySeries.monthlyRev:moD)}><CartesianGrid {...gl}/><XAxis dataKey="month" tick={tk}/><YAxis tick={tk} tickFormatter={fmtY}/><Tooltip content={<CT formatter={v=>"¥"+v.toLocaleString()}/>}/>{(perPropertySeries||facAgeSeries||facTypeSeries||perCountrySeries)?<Legend wrapperStyle={{fontSize:9}}/>:null}{perPropertySeries?perPropertySeries.properties.map((f,i)=><Bar key={f} dataKey={f} stackId="a" fill={seriesColor(f,i)} name={seriesFacLabel(f)}/>):facAgeSeries?[<Bar key="Old" dataKey="Old" stackId="a" fill={OLD_COHORT_COLOR} name={t.facOldLabel}/>,<Bar key="New" dataKey="New" stackId="a" fill={NEW_COHORT_COLOR} name={t.facNewLabel}/>]:facTypeSeries?[<Bar key="Apart" dataKey="Apart" stackId="a" fill={APART_COLOR} name={t.facApartLabel}/>,<Bar key="Hotel" dataKey="Hotel" stackId="a" fill={HOTEL_COLOR} name={t.facHotelLabel}/>]:perCountrySeries?perCountrySeries.countries.map((c,i)=><Bar key={c} dataKey={c} stackId="a" fill={seriesColor(c,i)} name={seriesLabel(c)}/>):<Bar dataKey="rev" fill="#34d399" radius={[4,4,0,0]} name={t.totalRevenue}/>}</BarChart></CC></div>
          ...
          <div key="ch-res-day"><CC grid title={t.resByDay} id="ch-res-day" nm="res_day" data={perPropertySeries?perPropertySeries.dailyCount:facAgeSeries?facAgeSeries.dailyCount:facTypeSeries?facTypeSeries.dailyCount:(perCountrySeries?perCountrySeries.dailyCount:dailyD)}><BarChart data={perPropertySeries?perPropertySeries.dailyCount:facAgeSeries?facAgeSeries.dailyCount:facTypeSeries?facTypeSeries.dailyCount:(perCountrySeries?perCountrySeries.dailyCount:dailyD)}><CartesianGrid {...gl}/><XAxis dataKey="date" tick={tks}/><YAxis tick={tk}/><Tooltip content={<CT/>}/>{(perPropertySeries||facAgeSeries||facTypeSeries||perCountrySeries)?<Legend wrapperStyle={{fontSize:9}}/>:null}{perPropertySeries?perPropertySeries.properties.map((f,i)=><Bar key={f} dataKey={f} stackId="a" fill={seriesColor(f,i)} name={seriesFacLabel(f)}/>):facAgeSeries?[<Bar key="Old" dataKey="Old" stackId="a" fill={OLD_COHORT_COLOR} name={t.facOldLabel}/>,<Bar key="New" dataKey="New" stackId="a" fill={NEW_COHORT_COLOR} name={t.facNewLabel}/>]:facTypeSeries?[<Bar key="Apart" dataKey="Apart" stackId="a" fill={APART_COLOR} name={t.facApartLabel}/>,<Bar key="Hotel" dataKey="Hotel" stackId="a" fill={HOTEL_COLOR} name={t.facHotelLabel}/>]:perCountrySeries?perCountrySeries.countries.map((c,i)=><Bar key={c} dataKey={c} stackId="a" fill={seriesColor(c,i)} name={seriesLabel(c)}/>):<Bar dataKey="count" fill="#4ea8de" radius={[4,4,0,0]} name={t.reservations}/>}</BarChart></CC></div>
          <div key="ch-rev-day"><CC grid title={t.revByDay} id="ch-rev-day" nm="rev_day" data={perPropertySeries?perPropertySeries.dailyRev:facAgeSeries?facAgeSeries.dailyRev:facTypeSeries?facTypeSeries.dailyRev:(perCountrySeries?perCountrySeries.dailyRev:dailyD)}><BarChart data={perPropertySeries?perPropertySeries.dailyRev:facAgeSeries?facAgeSeries.dailyRev:facTypeSeries?facTypeSeries.dailyRev:(perCountrySeries?perCountrySeries.dailyRev:dailyD)}><CartesianGrid {...gl}/><XAxis dataKey="date" tick={tks}/><YAxis tick={tk} tickFormatter={fmtY}/><Tooltip content={<CT formatter={v=>"¥"+v.toLocaleString()}/>}/>{(perPropertySeries||facAgeSeries||facTypeSeries||perCountrySeries)?<Legend wrapperStyle={{fontSize:9}}/>:null}{perPropertySeries?perPropertySeries.properties.map((f,i)=><Bar key={f} dataKey={f} stackId="a" fill={seriesColor(f,i)} name={seriesFacLabel(f)}/>):facAgeSeries?[<Bar key="Old" dataKey="Old" stackId="a" fill={OLD_COHORT_COLOR} name={t.facOldLabel}/>,<Bar key="New" dataKey="New" stackId="a" fill={NEW_COHORT_COLOR} name={t.facNewLabel}/>]:facTypeSeries?[<Bar key="Apart" dataKey="Apart" stackId="a" fill={APART_COLOR} name={t.facApartLabel}/>,<Bar key="Hotel" dataKey="Hotel" stackId="a" fill={HOTEL_COLOR} name={t.facHotelLabel}/>]:perCountrySeries?perCountrySeries.countries.map((c,i)=><Bar key={c} dataKey={c} stackId="a" fill={seriesColor(c,i)} name={seriesLabel(c)}/>):<Bar dataKey="rev" fill="#34d399" radius={[4,4,0,0]} name={t.totalRevenue}/>}</BarChart></CC></div>
        </DraggableGrid></>}
        ...
        {tab==="booking"&&<><DraggableGrid {...dgProps("booking")}>
          <div key="ch-bd"><CC grid title={t.ciCoDOW} id="ch-bd" nm="dow" h={300} data={perPropertySeries?perPropertySeries.dowCount:facAgeSeries?facAgeSeries.dowCount:facTypeSeries?facTypeSeries.dowCount:(perCountrySeries?perCountrySeries.dowCount:dowD)}><BarChart data={perPropertySeries?perPropertySeries.dowCount:facAgeSeries?facAgeSeries.dowCount:facTypeSeries?facTypeSeries.dowCount:(perCountrySeries?perCountrySeries.dowCount:dowD)}><CartesianGrid {...gl}/><XAxis dataKey="day" tick={tk}/><YAxis tick={tk}/><Tooltip content={<CT/>}/><Legend wrapperStyle={{fontSize:9}}/>{perPropertySeries?perPropertySeries.properties.map((f,i)=><Bar key={f} dataKey={f} stackId="a" fill={seriesColor(f,i)} name={seriesFacLabel(f)}/>):facAgeSeries?[<Bar key="Old" dataKey="Old" stackId="a" fill={OLD_COHORT_COLOR} name={t.facOldLabel}/>,<Bar key="New" dataKey="New" stackId="a" fill={NEW_COHORT_COLOR} name={t.facNewLabel}/>]:facTypeSeries?[<Bar key="Apart" dataKey="Apart" stackId="a" fill={APART_COLOR} name={t.facApartLabel}/>,<Bar key="Hotel" dataKey="Hotel" stackId="a" fill={HOTEL_COLOR} name={t.facHotelLabel}/>]:perCountrySeries?perCountrySeries.countries.map((c,i)=><Bar key={c} dataKey={c} stackId="a" fill={seriesColor(c,i)} name={seriesLabel(c)}/>):<><Bar dataKey="checkin" fill="#4ea8de" radius={[4,4,0,0]} name={t.checkInLabel}/><Bar dataKey="checkout" fill="#e07b54" radius={[4,4,0,0]} name={t.checkOutLabel}/></>}</BarChart></CC></div>
        ...
        {/* MEMBER */}
        {tab==="member"&&<>          <div style={{background:"rgba(217,119,6,0.14)",border:"2px solid #d97706",borderLeft:"6px solid #d97706",borderRadius:6,padding:"12px 16px",marginBottom:16,fontSize:13,fontWeight:600,color:TH.textStrong,lineHeight:1.55,display:"flex",gap:10,alignItems:"flex-start"}}><span style={{fontSize:20,lineHeight:1,flexShrink:0}}>⚠</span><span>{t.memberDisclaimer}</span></div>
          <div style={{display:"flex",gap:10,marginBottom:16,flexWrap:"wrap",flexDirection:isMobile?"column":"row"}}>
        ...
        {tab==="revenue"&&<><DraggableGrid {...dgProps("revenue")}>
          ...
          <div key="ch-rdow"><CC grid title={t.revByDOW} id="ch-rdow" nm="rev_dow" data={perPropertySeries?perPropertySeries.dowRev:facAgeSeries?facAgeSeries.dowRev:facTypeSeries?facTypeSeries.dowRev:(perCountrySeries?perCountrySeries.dowRev:revDowD)}><BarChart data={perPropertySeries?perPropertySeries.dowRev:facAgeSeries?facAgeSeries.dowRev:facTypeSeries?facTypeSeries.dowRev:(perCountrySeries?perCountrySeries.dowRev:revDowD)}><CartesianGrid {...gl}/><XAxis dataKey="day" tick={tk}/><YAxis tick={tk} tickFormatter={fmtY}/><Tooltip content={<CT formatter={v=>"¥"+v.toLocaleString()}/>}/>{(perPropertySeries||facAgeSeries||facTypeSeries||perCountrySeries)?<Legend wrapperStyle={{fontSize:9}}/>:null}{perPropertySeries?perPropertySeries.properties.map((f,i)=><Bar key={f} dataKey={f} stackId="a" fill={seriesColor(f,i)} name={seriesFacLabel(f)}/>):facAgeSeries?[<Bar key="Old" dataKey="Old" stackId="a" fill={OLD_COHORT_COLOR} name={t.facOldLabel}/>,<Bar key="New" dataKey="New" stackId="a" fill={NEW_COHORT_COLOR} name={t.facNewLabel}/>]:facTypeSeries?[<Bar key="Apart" dataKey="Apart" stackId="a" fill={APART_COLOR} name={t.facApartLabel}/>,<Bar key="Hotel" dataKey="Hotel" stackId="a" fill={HOTEL_COLOR} name={t.facHotelLabel}/>]:perCountrySeries?perCountrySeries.countries.map((c,i)=><Bar key={c} dataKey={c} stackId="a" fill={seriesColor(c,i)} name={seriesLabel(c)}/>):<Bar dataKey="rev" fill="#34d399" radius={[4,4,0,0]} name={t.totalRevenue}/>}</BarChart></CC></div>
        ...
        {tab==="tl-revenue"&&tlRevenueRpt&&<div>
          ...
          <DraggableGrid {...dgProps("tl-revenue")}>
            <div key="tlr-mo"><CC grid title="Monthly Revenue" id="tlr-mo" nm="tl_rev_mo" data={tlAgeSeries?tlAgeSeries.monthlyRev:tlTypeSeries?tlTypeSeries.monthlyRev:tlRevenueRpt.moRows}>
              {tlAgeSeries?<BarChart data={tlAgeSeries.monthlyRev}><CartesianGrid {...gl}/><XAxis dataKey="month" tick={tk}/><YAxis tick={tk} tickFormatter={fmtY}/><Tooltip content={<CT formatter={v=>"¥"+v.toLocaleString()}/>}/><Legend wrapperStyle={{fontSize:10}}/><Bar dataKey="Old" stackId="a" fill={OLD_COHORT_COLOR} name={t.facOldLabel}/><Bar dataKey="New" stackId="a" fill={NEW_COHORT_COLOR} name={t.facNewLabel}/></BarChart>:tlTypeSeries?<BarChart data={tlTypeSeries.monthlyRev}><CartesianGrid {...gl}/><XAxis dataKey="month" tick={tk}/><YAxis tick={tk} tickFormatter={fmtY}/><Tooltip content={<CT formatter={v=>"¥"+v.toLocaleString()}/>}/><Legend wrapperStyle={{fontSize:10}}/><Bar dataKey="Apart" stackId="a" fill={APART_COLOR} name={t.facApartLabel}/><Bar dataKey="Hotel" stackId="a" fill={HOTEL_COLOR} name={t.facHotelLabel}/></BarChart>:<ComposedChart data={tlRevenueRpt.moRows}><CartesianGrid {...gl}/><XAxis dataKey="month" tick={tk}/><YAxis tick={tk} tickFormatter={fmtY}/><Tooltip content={<CT formatter={v=>"¥"+v.toLocaleString()}/>}/><Legend wrapperStyle={{fontSize:10}}/><Bar dataKey="rev" fill={SOURCE_COLORS.tl} radius={[4,4,0,0]} name={t.tlMetricRev}/></ComposedChart>}
            </CC></div>
            <div key="tlr-daily"><CC grid title="Daily Revenue" id="tlr-daily" nm="tl_rev_daily" data={tlAgeSeries?tlAgeSeries.dailyRev:tlTypeSeries?tlTypeSeries.dailyRev:tlRevenueRpt.dayRows}>
              {tlAgeSeries?<BarChart data={tlAgeSeries.dailyRev}><CartesianGrid {...gl}/><XAxis dataKey="date" tick={tks}/><YAxis tick={tk} tickFormatter={fmtY}/><Tooltip content={<CT formatter={v=>"¥"+v.toLocaleString()}/>}/><Legend wrapperStyle={{fontSize:10}}/><Bar dataKey="Old" stackId="a" fill={OLD_COHORT_COLOR} name={t.facOldLabel}/><Bar dataKey="New" stackId="a" fill={NEW_COHORT_COLOR} name={t.facNewLabel}/></BarChart>:tlTypeSeries?<BarChart data={tlTypeSeries.dailyRev}><CartesianGrid {...gl}/><XAxis dataKey="date" tick={tks}/><YAxis tick={tk} tickFormatter={fmtY}/><Tooltip content={<CT formatter={v=>"¥"+v.toLocaleString()}/>}/><Legend wrapperStyle={{fontSize:10}}/><Bar dataKey="Apart" stackId="a" fill={APART_COLOR} name={t.facApartLabel}/><Bar dataKey="Hotel" stackId="a" fill={HOTEL_COLOR} name={t.facHotelLabel}/></BarChart>:<LineChart data={tlRevenueRpt.dayRows}><CartesianGrid {...gl}/><XAxis dataKey="date" tick={tks}/><YAxis tick={tk} tickFormatter={fmtY}/><Tooltip content={<CT formatter={v=>"¥"+v.toLocaleString()}/>}/><Line type="monotone" dataKey="rev" stroke={SOURCE_COLORS.tl} strokeWidth={1.5} dot={false} name={t.tlMetricRev}/></LineChart>}
            </CC></div>
            ...
            <div key="tlr-dow"><CC grid title="Revenue by Day-of-Week" id="tlr-dow" nm="tl_rev_dow" data={tlAgeSeries?tlAgeSeries.dowRev:tlTypeSeries?tlTypeSeries.dowRev:tlRevenueRpt.dowRows}>
              <BarChart data={tlAgeSeries?tlAgeSeries.dowRev:tlTypeSeries?tlTypeSeries.dowRev:tlRevenueRpt.dowRows}><CartesianGrid {...gl}/><XAxis dataKey="day" tick={tk}/><YAxis tick={tk} tickFormatter={fmtY}/><Tooltip content={<CT formatter={v=>"¥"+v.toLocaleString()}/>}/>{(tlAgeSeries||tlTypeSeries)?<Legend wrapperStyle={{fontSize:9}}/>:null}{tlAgeSeries?[<Bar key="Old" dataKey="Old" stackId="a" fill={OLD_COHORT_COLOR} name={t.facOldLabel}/>,<Bar key="New" dataKey="New" stackId="a" fill={NEW_COHORT_COLOR} name={t.facNewLabel}/>]:tlTypeSeries?[<Bar key="Apart" dataKey="Apart" stackId="a" fill={APART_COLOR} name={t.facApartLabel}/>,<Bar key="Hotel" dataKey="Hotel" stackId="a" fill={HOTEL_COLOR} name={t.facHotelLabel}/>]:<Bar dataKey="rev" fill={SOURCE_COLORS.tl} radius={[4,4,0,0]} name={t.tlMetricRev}/>}</BarChart>
            </CC></div>
          </DraggableGrid>
        </div>}
        ...
        {tab==="tl-overview"&&tlOverviewRpt&&<DraggableGrid {...dgProps("tl-overview")}>
          <div key="tlo-mo"><CC grid title="Monthly Reservations" id="tlo-mo" nm="tl_ov_mo" data={tlAgeSeries?tlAgeSeries.monthlyCount:tlTypeSeries?tlTypeSeries.monthlyCount:tlOverviewRpt.moRows}><BarChart data={tlAgeSeries?tlAgeSeries.monthlyCount:tlTypeSeries?tlTypeSeries.monthlyCount:tlOverviewRpt.moRows}><CartesianGrid {...gl}/><XAxis dataKey="month" tick={tk}/><YAxis tick={tk}/><Tooltip content={<CT/>}/>{(tlAgeSeries||tlTypeSeries)?<Legend wrapperStyle={{fontSize:9}}/>:null}{tlAgeSeries?[<Bar key="Old" dataKey="Old" stackId="a" fill={OLD_COHORT_COLOR} name={t.facOldLabel}/>,<Bar key="New" dataKey="New" stackId="a" fill={NEW_COHORT_COLOR} name={t.facNewLabel}/>]:tlTypeSeries?[<Bar key="Apart" dataKey="Apart" stackId="a" fill={APART_COLOR} name={t.facApartLabel}/>,<Bar key="Hotel" dataKey="Hotel" stackId="a" fill={HOTEL_COLOR} name={t.facHotelLabel}/>]:<Bar dataKey="count" fill={SOURCE_COLORS.tl} radius={[4,4,0,0]} name={t.reservations}/>}</BarChart></CC></div>
          ...
          <div key="tlo-dow"><CC grid title="Check-in DOW" id="tlo-dow" nm="tl_ov_dow" data={tlAgeSeries?tlAgeSeries.dowCount:tlTypeSeries?tlTypeSeries.dowCount:tlOverviewRpt.dowRows}><BarChart data={tlAgeSeries?tlAgeSeries.dowCount:tlTypeSeries?tlTypeSeries.dowCount:tlOverviewRpt.dowRows}><CartesianGrid {...gl}/><XAxis dataKey="day" tick={tk}/><YAxis tick={tk}/><Tooltip content={<CT/>}/>{(tlAgeSeries||tlTypeSeries)?<Legend wrapperStyle={{fontSize:9}}/>:null}{tlAgeSeries?[<Bar key="Old" dataKey="Old" stackId="a" fill={OLD_COHORT_COLOR} name={t.facOldLabel}/>,<Bar key="New" dataKey="New" stackId="a" fill={NEW_COHORT_COLOR} name={t.facNewLabel}/>]:tlTypeSeries?[<Bar key="Apart" dataKey="Apart" stackId="a" fill={APART_COLOR} name={t.facApartLabel}/>,<Bar key="Hotel" dataKey="Hotel" stackId="a" fill={HOTEL_COLOR} name={t.facHotelLabel}/>]:<Bar dataKey="count" fill={SOURCE_COLORS.tl} radius={[4,4,0,0]} name={t.reservations}/>}</BarChart></CC></div>
          <div key="tlo-mo-rev"><CC grid title="Monthly Revenue (税抜)" id="tlo-mo-rev" nm="tl_ov_mo_rev" data={tlAgeSeries?tlAgeSeries.monthlyRev:tlTypeSeries?tlTypeSeries.monthlyRev:tlOverviewRpt.moRows}>{tlAgeSeries?<BarChart data={tlAgeSeries.monthlyRev}><CartesianGrid {...gl}/><XAxis dataKey="month" tick={tk}/><YAxis tick={tk} tickFormatter={fmtY}/><Tooltip content={<CT formatter={v=>"¥"+v.toLocaleString()}/>}/><Legend wrapperStyle={{fontSize:9}}/><Bar dataKey="Old" stackId="a" fill={OLD_COHORT_COLOR} name={t.facOldLabel}/><Bar dataKey="New" stackId="a" fill={NEW_COHORT_COLOR} name={t.facNewLabel}/></BarChart>:tlTypeSeries?<BarChart data={tlTypeSeries.monthlyRev}><CartesianGrid {...gl}/><XAxis dataKey="month" tick={tk}/><YAxis tick={tk} tickFormatter={fmtY}/><Tooltip content={<CT formatter={v=>"¥"+v.toLocaleString()}/>}/><Legend wrapperStyle={{fontSize:9}}/><Bar dataKey="Apart" stackId="a" fill={APART_COLOR} name={t.facApartLabel}/><Bar dataKey="Hotel" stackId="a" fill={HOTEL_COLOR} name={t.facHotelLabel}/></BarChart>:<LineChart data={tlOverviewRpt.moRows}><CartesianGrid {...gl}/><XAxis dataKey="month" tick={tk}/><YAxis tick={tk} tickFormatter={fmtY}/><Tooltip content={<CT formatter={v=>"¥"+v.toLocaleString()}/>}/><Line type="monotone" dataKey="rev" stroke={SOURCE_COLORS.tl} strokeWidth={2} dot={{r:3}} name={t.tlMetricRev}/></LineChart>}</CC></div>
        </DraggableGrid>}
        ...
        {tab==="tl-booking"&&tlBookingRpt&&<DraggableGrid {...dgProps("tl-booking")}>
          ...
          <div key="tlb-dow"><CC grid title="Check-in DOW" id="tlb-dow" nm="tl_book_dow" data={tlAgeSeries?tlAgeSeries.dowCount:tlTypeSeries?tlTypeSeries.dowCount:tlBookingRpt.dowRows}><BarChart data={tlAgeSeries?tlAgeSeries.dowCount:tlTypeSeries?tlTypeSeries.dowCount:tlBookingRpt.dowRows}><CartesianGrid {...gl}/><XAxis dataKey="day" tick={tk}/><YAxis tick={tk}/><Tooltip content={<CT/>}/>{(tlAgeSeries||tlTypeSeries)?<Legend wrapperStyle={{fontSize:9}}/>:null}{tlAgeSeries?[<Bar key="Old" dataKey="Old" stackId="a" fill={OLD_COHORT_COLOR} name={t.facOldLabel}/>,<Bar key="New" dataKey="New" stackId="a" fill={NEW_COHORT_COLOR} name={t.facNewLabel}/>]:tlTypeSeries?[<Bar key="Apart" dataKey="Apart" stackId="a" fill={APART_COLOR} name={t.facApartLabel}/>,<Bar key="Hotel" dataKey="Hotel" stackId="a" fill={HOTEL_COLOR} name={t.facHotelLabel}/>]:<Bar dataKey="count" fill={SOURCE_COLORS.tl} radius={[4,4,0,0]} name={t.reservations}/>}</BarChart></CC></div>
          ...
          {tlCompareRpt&&!tlCompareRpt.empty?<div>
            ...
            <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"1fr 1fr",gap:14}}>
              <SortTbl
                ...
                columns={[{key:"country",label:"Country"},{key:"revA",label:"A ¥",headerColor:CMP_A_COLOR},{key:"nightsA",label:"A "+t.roomNights,headerColor:CMP_A_COLOR},{key:"revB",label:"B ¥",headerColor:CMP_B_COLOR},{key:"nightsB",label:"B "+t.roomNights,headerColor:CMP_B_COLOR},{key:"revDelta",label:"Δ ¥"},{key:"nightsDelta",label:"Δ "+t.roomNights}]}
              ...
              <SortTbl
                ...
                columns={[{key:"segment",label:"Segment"},{key:"revA",label:"A ¥",headerColor:CMP_A_COLOR},{key:"nightsA",label:"A "+t.roomNights,headerColor:CMP_A_COLOR},{key:"revB",label:"B ¥",headerColor:CMP_B_COLOR},{key:"nightsB",label:"B "+t.roomNights,headerColor:CMP_B_COLOR},{key:"revDelta",label:"Δ ¥"},{key:"nightsDelta",label:"Δ "+t.roomNights}]}
              ...
              <div style={{gridColumn:isMobile?"auto":"span 2"}}><SortTbl
                ...
                columns={[{key:"name",label:"Facility"},{key:"revA",label:"A ¥",headerColor:CMP_A_COLOR},{key:"nightsA",label:"A "+t.roomNights,headerColor:CMP_A_COLOR},{key:"revB",label:"B ¥",headerColor:CMP_B_COLOR},{key:"nightsB",label:"B "+t.roomNights,headerColor:CMP_B_COLOR},{key:"revDelta",label:"Δ ¥"},{key:"nightsDelta",label:"Δ "+t.roomNights}]}
